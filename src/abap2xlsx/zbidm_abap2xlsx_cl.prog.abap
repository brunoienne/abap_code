...
PARAMETERS: p_email TYPE string LOWER CASE MODIF ID ema OBLIGATORY DEFAULT ''.
...
* Get current value
    ...
    cl_gui_frontend_services=>directory_browse(
      EXPORTING
        window_title         = 'Selecione o diretório para o caminho Excel'
...
* Furthermore I don't have to change the selectiontexts of all demoreports.
    ...
    default_parametertext:  rb_down  'Salvar no frontend',
                            rb_back  'Salvar no backend',
                            rb_show  'Exibir em tela',
                            rb_send  'Enviar via email',

                            p_path   'Diretório para salvar arquivo',
                            p_email  'Email para envio do xlsx'.
...
* Get trailing blank
  ...
  METHOD download_backend.
    ...
    IF sy-repid <> sy-cprog AND sy-cprog IS NOT INITIAL.  " no need to display anything if download was selected and report was called for demo purposes
      LEAVE PROGRAM.
    ELSE.
      MESSAGE 'Dados transferidos para o diretório padrão' TYPE 'S'.
    ENDIF.
...
* Needed to send emails
    ...
    mail_title     = 'Título do email'.
    wa_mailtext    = 'Texto do email'.
...
* Und abschicken
        ...
        ELSE.
          MESSAGE s022(so).
          MESSAGE 'Documento pronto para envio - Verifique SOST ou SCOT' TYPE 'I'.
        ENDIF.
